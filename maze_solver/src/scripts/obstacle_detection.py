...
def isValidPoint(parentNodeX, parentNodeY, childNodeX, childNodeY):
    ...
    for wall in maze:
        ...
        while i < 3:
            ...
            i += 1
    return valid_point
start_point = Point(-5.2,-2.6)
end_point = Point(3.2,0.27)

points = [start_point] 

def shortest_node(point_list,point):
    n = len(point_list)
    dist_list = []
    for i in range(n):
        dist = mt.sqrt((point_list[i].x - point.x)**2 + (point_list[i].y - point.y)**2)
        dist_list.append(dist)
    min_dist = min(dist_list)
    for i in range(n):
        dist = mt.sqrt((point_list[i].x - point.x)**2 + (point_list[i].y - point.y)**2)
        if(min_dist == dist):
            shortest_node = point_list[i]
    return shortest_node

n = 3000
l = 0.15
for i in range(n):
    x_rand = random.randrange(-6,6)
    y_rand = random.randrange(-2,3)
    init_point = shortest_node(points , Point(x_rand,y_rand))
    slope = np.arctan((y_rand - init_point.y)/(x_rand - init_point.x))
    x_node = init_point.x + l*mt.cos(slope)
    y_node = init_point.y + l*mt.sin(slope)
    point_new = Point(x_node,y_node)
    if(isValidPoint(point_new.x , point_new.y , init_point.x , init_point.y)):
